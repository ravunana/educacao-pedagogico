...
/**
 * Performance test for the Chamada entity.
 */
class ChamadaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Chamada entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all chamadas")
            .get("/services/pedagogico/api/chamadas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new chamada")
            .post("/services/pedagogico/api/chamadas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_chamada_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created chamada")
                .get("/services/pedagogico${new_chamada_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created chamada")
            .delete("/services/pedagogico${new_chamada_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
