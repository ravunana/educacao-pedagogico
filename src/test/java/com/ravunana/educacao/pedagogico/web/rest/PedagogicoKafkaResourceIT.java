...
import com.ravunana.educacao.pedagogico.config.KafkaProperties;
...
class PedagogicoKafkaResourceIT {
    ...
    void setup() {
        ...
        PedagogicoKafkaResource kafkaResource = new PedagogicoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pedagogico-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/pedagogico-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
